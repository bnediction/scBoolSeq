...
from statsmodels.discrete.discrete_model import (
    Poisson, GeneralizedPoisson, # Counts 
    NegativeBinomial, NegativeBinomialP, # idem. var > mu
...
class SizeFactorRegressor(RegressorMixin, BaseEstimator):
    ...
    def __init__(
        ...
        self.preproc_transformer = preproc_transformer
    ...
    def predict(self, X):
        ...
        return (
            self.glm_fitted_
                .predict(X)
                .rename(self.glm_spec_.endog_names)
                .astype(int)
        )
    ...
    def score(self, X, y):
        ...
        error_frame.loc[:, "rel_error"] = np.abs(error_frame['error']) / error_frame[y.name]
        return error_frame
    ...
    def de_normalize(self, X, target_sum):
        ...
        return (np.expm1(X) / target_sum).T.apply(lambda x: x * _new_size_factors[x.name]).astype(int)
